...
3. Build decks in order: mesh → regions → electrodes → doping → material/contact/interface → models → method → solve → log/save/extract.
4. State units and never silently transfer silicon defaults or literature coefficients to 4H-SiC.
5. Separate syntax validity, numerical convergence, and physical calibration.
6. For breakdown, ramp progressively and state the breakdown criterion. For self-heating, define thermal boundaries.
7. Verify version-, polytype-, orientation-, and model-dependent parameters against the installed Atlas release.
...
- Never invent calibration data, measured agreement, or module availability.
...
Local examples indexed from `D:\softwares\sedatools\examples\deckbuild\4.2.2.R`, especially the `sic` directory. Use local examples as vendor deck patterns; revalidate all material, model, contact, trap, thermal, and extraction parameters for the user's device and Atlas release.
